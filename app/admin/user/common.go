...
func GetMenuArray(pdata []gform.Data, parent_id int64, roles []interface{}) []gform.Data {
	...
	for _, v := range pdata {
		if v["pid"].(int64) == parent_id {
			...
			//9.权限
			if len(roles) == 0 {
				permission, _ := model.DB().Table("admin_auth_rule").Where("type", 2).Where("pid", v["id"]).Pluck("permission")
				if permission != nil && len(permission.([]interface{})) > 0 {
					meta["roles"] = permission
				}
			} else {
				permission, _ := model.DB().Table("admin_auth_rule").Where("type", 2).Where("pid", v["id"]).WhereIn("id", roles).Pluck("permission")
				if permission != nil && len(permission.([]interface{})) > 0 {
					meta["roles"] = permission
				} else {
					meta["roles"] = [1]string{"*"}
