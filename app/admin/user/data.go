...
func (api *Data) Get_user(c *gin.Context) {
	...
	data, err := model.DB().Table("admin_user").Where("id", user.ID).Fields("id,status,nickname,name,mobile,email,company,remark,city,area,address,createtime").First()
	if err != nil {
...
func (api *Data) CheckPassword(c *gin.Context) {
	...
	getuser, _ := c.Get("user") //取值 实现了跨中间件取值
	user := getuser.(*middleware.UserClaims)
	data, err := model.DB().Table("admin_user").Where("id", user.ID).Fields("password,salt").First()
	if err != nil {
