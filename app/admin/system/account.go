...
func (api *Account) Get_list(c *gin.Context) {
	...
	MDB := model.DB().Table("admin_user").Fields("id,status,name,username,avatar,tel,mobile,email,remark,dept_id,remark,city,address,company,createtime").
	...
	if err != nil {
		...
		totalCount, _ = model.DB().Table("admin_user").Count()
...
func (api *Account) Save(c *gin.Context) {
	...
	if f_id == 0 {
		...
		addId, err := model.DB().Table("admin_user").Data(parameter).InsertGetId()
		if err != nil {
...
func (api *Account) UpStatus(c *gin.Context) {
	...
	var parameter map[string]interface{}
	_ = json.Unmarshal(body, &parameter)
	res2, err := model.DB().Table("admin_user").Where("id", parameter["id"]).Data(map[string]interface{}{"status": parameter["status"]}).Update()
	if err != nil {
		results.Failed(c, "更新失败！", err)
	} else {
		msg := "更新成功！"
...
func (api *Account) Del(c *gin.Context) {
	...
	_ = json.Unmarshal(body, &parameter)
	ids := parameter["ids"]
	res2, err := model.DB().Table("admin_user").WhereIn("id", ids.([]interface{})).Delete()
	if err != nil {
...
func (api *Account) Get_account(c *gin.Context) {
	getuser, _ := c.Get("user")
	user := getuser.(*middleware.UserClaims)
	data, _ := model.DB().Table("admin_user").Where("id", user.ID).First()
...
func (api *Account) Get_role(c *gin.Context) {
	...
	role_ids := utils.GetAllChilIds("admin_auth_role", role_id.([]interface{})) //批量获取子节点id
...
func (api *Account) Isaccountexist(c *gin.Context) {
	...
	_ = json.Unmarshal(body, &parameter)
	if parameter["id"] != nil {
		res1, err := model.DB().Table("admin_user").Where("id", "!=", parameter["id"]).Where("username", parameter["username"]).Value("id")
		if err != nil {
		...
		res2, err := model.DB().Table("admin_user").Where("username", parameter["username"]).Value("id")
		if err != nil {
			results.Failed(c, "验证失败", err)
		} else if res2 != nil {
			results.Failed(c, "账号已存在", err)
