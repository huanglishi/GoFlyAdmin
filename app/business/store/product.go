...
func (api *Product) Save(c *gin.Context) {
	...
	if f_id == 0 {
		...
		addId, err := model.DB().Table("business_store_product").Data(parameter).InsertGetId()
		if err != nil {
			results.Failed(c, "添加失败", err)
		} else {
			if addId != 0 {
				model.DB().Table("business_store_product").
		...
		res, err := model.DB().Table("business_store_product").
			Data(parameter).
			Where("id", f_id).
			Update()
		if err != nil {
...
func (api *Product) UpStatus(c *gin.Context) {
	...
	var parameter map[string]interface{}
	_ = json.Unmarshal(body, &parameter)
	res2, err := model.DB().Table("business_store_product").Where("id", parameter["id"]).Data(map[string]interface{}{"status": parameter["status"]}).Update()
	if err != nil {
		results.Failed(c, "更新失败！", err)
	} else {
		msg := "更新成功！"
...
func (api *Product) Del(c *gin.Context) {
	...
	_ = json.Unmarshal(body, &parameter)
	ids := parameter["ids"]
	res2, err := model.DB().Table("business_store_product").WhereIn("id", ids.([]interface{})).Delete()
	if err != nil {
...
func (api *Product) Get_content(c *gin.Context) {
	...
	if id == "" {
		...
		res2, err := model.DB().Table("business_store_product").Where("id", id).First()
		if err != nil {
