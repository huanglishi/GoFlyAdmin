...
func (api *Account) Get_userdata(c *gin.Context) {
	...
	userdata, err := model.DB().Table("business_account").Fields("id,accountID,username,nickname,avatar,status,password,salt,tel,email,city,remark").Where("id", user.ID).First()
	if err != nil {
...
func (api *Account) Get_menu(c *gin.Context) {
	...
	user := getuser.(*middleware.UserClaims)
	//获取用户权限菜单
	role_id, acerr := model.DB().Table("business_auth_role_access").Where("uid", user.ID).Pluck("role_id")
	if acerr != nil {
	...
	menu_ids, rerr := model.DB().Table("business_auth_role").WhereIn("id", role_id.([]interface{})).Pluck("rules")
	if rerr != nil {
...
func (api *Account) Upavatar(c *gin.Context) {
	...
	getuser, _ := c.Get("user") //取值 实现了跨中间件取值
	user := getuser.(*middleware.UserClaims)
	res, err := model.DB().Table("business_account").Where("id", user.ID).Data(map[string]interface{}{"avatar": parameter["url"]}).Update()
	if err != nil {
...
func (api *Account) Changepwd(c *gin.Context) {
	...
	getuser, _ := c.Get("user") //取值 实现了跨中间件取值
	user := getuser.(*middleware.UserClaims)
	userdata, err := model.DB().Table("business_account").Where("id", user.ID).Fields("password,salt").First()
	if err != nil {
