...
func (api *Account) Get_userdata(c *gin.Context) {
	...
	userdata, err := model.DB().Table("business_user").Fields("id,accountID,username,nickname,avatar,status,password,salt,tel,email,city,remark").Where("id", user.ID).First()
	if err != nil {
...
func (api *Account) Upavatar(c *gin.Context) {
	...
	getuser, _ := c.Get("user") //取值 实现了跨中间件取值
	user := getuser.(*middleware.UserClaims)
	res, err := model.DB().Table("business_user").Where("id", user.ID).Data(map[string]interface{}{"avatar": parameter["url"]}).Update()
	if err != nil {
...
func (api *Account) Changepwd(c *gin.Context) {
	...
	getuser, _ := c.Get("user") //取值 实现了跨中间件取值
	user := getuser.(*middleware.UserClaims)
	userdata, err := model.DB().Table("business_user").Where("id", user.ID).Fields("password,salt").First()
	if err != nil {
