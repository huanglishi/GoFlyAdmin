...
// 获取列表
func (api *Code) Get_list(c *gin.Context) {
...
// 保存
func (api *Code) Save(c *gin.Context) {
	...
	if f_id == 0 {
		delete(parameter, "id")
		if utils.IsHaseField("business_createcode_code", "createtime") {
			parameter["createtime"] = time.Now().Unix()
		}
		if utils.IsHaseField("business_createcode_code", "businessID") {
			parameter["businessID"] = user.BusinessID
		}
		addId, err := model.DB().Table("business_createcode_code").Data(parameter).InsertGetId()
		if err != nil {
			results.Failed(c, "添加失败", err)
		} else {
			if addId != 0 {
				model.DB().Table("business_createcode_code").
		...
		res, err := model.DB().Table("business_createcode_code").
			Data(parameter).
			Where("id", f_id).
			Update()
		if err != nil {
...
// 更新状态
func (api *Code) UpStatus(c *gin.Context) {
	body, _ := ioutil.ReadAll(c.Request.Body)
	var parameter map[string]interface{}
	_ = json.Unmarshal(body, &parameter)
	res2, err := model.DB().Table("business_createcode_code").Where("id", parameter["id"]).Data(map[string]interface{}{"status": parameter["status"]}).Update()
	if err != nil {
		results.Failed(c, "更新失败！", err)
	} else {
		msg := "更新成功！"
...
// 删除
func (api *Code) Del(c *gin.Context) {
	body, _ := ioutil.ReadAll(c.Request.Body)
	var parameter map[string]interface{}
	_ = json.Unmarshal(body, &parameter)
	ids := parameter["ids"]
	res2, err := model.DB().Table("business_createcode_code").WhereIn("id", ids.([]interface{})).Delete()
	if err != nil {
...
// 获取内容
func (api *Code) Get_content(c *gin.Context) {
	id := c.DefaultQuery("id", "")
	if id == "" {
		results.Failed(c, "请传参数id", nil)
	} else {
		res2, err := model.DB().Table("business_createcode_code").Where("id", id).First()
		if err != nil {
