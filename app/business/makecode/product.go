...
// 获取列表
func (api *Product) Get_list(c *gin.Context) {
	 //请求参数
	params, _ := gf.RequestParam(c)
	MDB := model.DB().Table("createcode_product").
		Fields("id,title,price,num,createtime")
	MDBC := model.DB().Table("createcode_product")
	if gf.IsHaseField("createcode_product", "businessID") {
	...
	if err != nil {
		...
		for _, val := range list {
			...
			if _, ok := val["cid"]; ok {
				catename, _ := model.DB().Table("").Where("id", val["cid"]).Value("name")
...
// 保存
func (api *Product) Save(c *gin.Context) {
	...
	if f_id == 0 {
		delete(parameter, "id")
		if gf.IsHaseField("createcode_product", "createtime") {
			parameter["createtime"] = time.Now().Unix()
		}
		if gf.IsHaseField("createcode_product", "businessID") {
			parameter["businessID"] = user.BusinessID
		}
		addId, err := model.DB().Table("createcode_product").Data(parameter).InsertGetId()
		if err != nil {
			results.Failed(c, "添加失败", err)
		} else {
			if addId != 0 {
				model.DB().Table("createcode_product").
		...
		res, err := model.DB().Table("createcode_product").
			Data(parameter).
			Where("id", f_id).
			Update()
		if err != nil {
...
// 更新状态
func (api *Product) UpStatus(c *gin.Context) {
	body, _ := io.ReadAll(c.Request.Body)
	var parameter map[string]interface{}
	_ = json.Unmarshal(body, &parameter)
	res2, err := model.DB().Table("createcode_product").Where("id", parameter["id"]).Data(map[string]interface{}{"status": parameter["status"]}).Update()
	if err != nil {
		results.Failed(c, "更新失败！", err)
	} else {
		msg := "更新成功！"
...
// 删除
func (api *Product) Del(c *gin.Context) {
	body, _ := io.ReadAll(c.Request.Body)
	var parameter map[string]interface{}
	_ = json.Unmarshal(body, &parameter)
	ids := parameter["ids"]
	res2, err := model.DB().Table("createcode_product").WhereIn("id", ids.([]interface{})).Delete()
	if err != nil {
...
// 获取内容
func (api *Product) Get_content(c *gin.Context) {
	id := c.DefaultQuery("id", "")
	if id == "" {
		results.Failed(c, "请传参数id", nil)
	} else {
		res2, err := model.DB().Table("createcode_product").Where("id", id).First()
		if err != nil {
