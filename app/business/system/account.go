...
func (api *Account) Get_list(c *gin.Context) {
	...
	MDB := model.DB().Table("business_account").Fields("id,status,name,username,avatar,tel,mobile,email,remark,dept_id,remark,city,address,company,createtime").
	...
	if err != nil {
		...
		for _, val := range list {
			...
			val["roleid"] = roleid
			depname, _ := model.DB().Table("business_auth_dept").Where("id", val["dept_id"]).Value("name")
			val["depname"] = depname
			//头像
			if val["avatar"] == nil {
