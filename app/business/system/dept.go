...
func (api *Dept) Get_list(c *gin.Context) {
	...
	MDB := model.DB().Table("business_auth_dept").Where("businessID", user.BusinessID)
	if name != "" {
...
func (api *Dept) Save(c *gin.Context) {
	...
	if f_id == 0 {
		...
		addId, err := model.DB().Table("business_auth_dept").Data(parameter).InsertGetId()
		if err != nil {
			results.Failed(c, "添加失败", err)
		} else {
			if addId != 0 {
				model.DB().Table("business_auth_dept").
...
func (api *Dept) UpStatus(c *gin.Context) {
	...
	var parameter map[string]interface{}
	_ = json.Unmarshal(body, &parameter)
	res2, err := model.DB().Table("business_auth_dept").Where("id", parameter["id"]).Data(map[string]interface{}{"status": parameter["status"]}).Update()
	if err != nil {
		results.Failed(c, "更新失败！", err)
	} else {
		msg := "更新成功！"
...
func (api *Dept) Upgrouppid(c *gin.Context) {
	...
	res2, err := model.DB().Table("business_auth_dept").WhereIn("id", ids_arr).Data(map[string]interface{}{"pid": parameter["pid"]}).Update()
	if err != nil {
		results.Failed(c, "更新失败！", err)
	} else {
		msg := "更新成功！"
...
func (api *Dept) Del(c *gin.Context) {
	...
	_ = json.Unmarshal(body, &parameter)
	ids := parameter["ids"]
	res2, err := model.DB().Table("business_auth_dept").WhereIn("id", ids.([]interface{})).Delete()
	if err != nil {
